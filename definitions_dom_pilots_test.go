...
func TestCanonicalDefinitionsDOMPilotsOffline(t *testing.T) {
	...
	for _, pilot := range []struct {
		...
		{name: "medium", host: "medium.com", yamlHost: "medium.com"},
		{name: "theverge", host: "www.theverge.com", yamlHost: "theverge.com"},
	} {
				...
				t.Run(test.File, func(t *testing.T) {
					...
					for _, format := range []string{"html", "markdown", "text"} {
						client := New(WithDefinitions(s), WithContentType(format), WithTransport(definitionTransport(func(r *http.Request) (*http.Response, error) {
							return &http.Response{StatusCode: http.StatusOK, Header: http.Header{"Content-Type": {"text/html"}}, Body: io.NopCloser(strings.NewReader(inputHTML)), Request: r}, nil
						})))
						for _, op := range []string{"Parse", "ParseHTML"} {
							...
							if op == "Parse" {
								...
								result, err = client.ParseHTML(context.Background(), inputHTML, remap(test.URL))
							}
							if err != nil {
								t.Fatal(err)
							}
